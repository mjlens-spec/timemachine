...
description: >-
  引导式营销方法论程序（不是交付引擎）。用一条固定路径，带使用者从 brief 走过「研判定位 → 反直觉洞察 → 战术推演 → 数据测算」，
  在过程里逼出他自己的判断、沉淀方法，顺带得到结果。底层逻辑是量化悖论：优秀做法会 alpha 衰减，所以要研判格局、找空位、做反直觉的差异化。
  研判与推演阶段以提问引导用户判断，研究与测算阶段 AI 重产出。每个项目结构化写入 projects/<client>-<date>/，按确认直推 GitHub main。
  当用户说「做洞察 / timemachine / 时间机器 / 接到 brief 怎么破题 / 帮我想营销打法」时使用。
when_to_use: >-
  收到营销 brief 或营销命题，想用固定方法论从研判定位一路走到反直觉洞察、战术推演与数据测算，
...
**铁律**：在 🧭 引导段（阶段 1、阶段 5），AI 不许大段替用户下结论。一旦开始替用户铺满答案，就是滑回了交付引擎，纠正回来。

## 在整套作业里的位置

```
客户 brief →【时间机器：研判定位 → Brief 解码 → 研究综述 → 反直觉洞察 → 激荡推演 → 数据测算】→ [成稿/PPT/演讲：SOP STEP 9–13]
```
...
- `references/methodology.md` — 量化悖论 + 研判定位 + 思维转弯 + 人机契约（阶段 1 必读）
- `references/research-playbook.md` — 多 Subagent fan-out + 平台数据取数（阶段 3）
- `references/insight-engine.md` — 四把刀 + A→D 共创 + 反驳（阶段 4 必读）
- `references/counter-intuition-bar.md` — 六点反直觉标尺 + 反模式
- `references/tactical-deduction.md` — 多 Option 激荡 + 选向推演（阶段 5）
- `references/calculation.md` — 触达成本 / 人群种草 / 预算分配 三块测算（阶段 6）
...
## 阶段 1 · 研判与定位 🧭 引导　★方法论核心入口

> 必读 `references/methodology.md`。这一段 AI **先问后给**，把判断逼给用户。

- **做**：① 扫描竞品 + 平台核心 Message（建坐标系，不是抄）。② 用品类心智坐标定位本品角色 + 找**可占领的空位**。③ **思维转弯检查点**：当场拦住「照搬优秀做法」的本能，问「它衰减到哪了？反面/盲区在哪？我有没有资格占空位？」。**转弯不写下来不准进阶段 2。**
- **产出**：`00-positioning.md`（用 `templates/positioning-map.md`）。

## 阶段 2 · Brief 解码 + 数据营销 Brief ⚙️ 执行

- **做**：① 用 `templates/brief-decode.md` 跑出信息完整性审查 / 补充问题清单(🔴🟡🟢) / 客户数据索取清单 + 时间机器加项『品类常识清单』『初步张力假设』（弹药接阶段 1 的转弯结论）。② 产出 `templates/data-marketing-brief.md`：一份给**数据营销同事**的取数单（云图/灵犀/星图按空位定向取数，不要全量）。
- **产出**：`01-brief-decode.md` + `02-data-marketing-brief.md`。

## 阶段 3 · 研究综述（多 Subagent fan-out）🛠 产出

> 细节见 `references/research-playbook.md`。

- **做**：`INGEST` 吃已有 Research Brief，只对矛盾点定向补搜；`LEAN` 派**多个子 agent**（`Agent`）各查一路（品类趋势 / 竞品公开动作 / 文化位移 / 人群讨论），汇成综述 §0；吃回数据同事的 A1 回传；平台后台数据 Claude 登不了 → 用粘贴模板请用户粘，当场打级别、原始件落 `_raw/`。§8 风险反证、§10 异常清单**必须非空**。
- **产出**：`03-research-synthesis.md`（用 `templates/research-synthesis.md`）。

## 阶段 4 · 反直觉洞察 · 共创 🤝 共创　★核心

> 完整玩法读 `references/insight-engine.md`。弹药＝`03 §10` 异常清单 + `00`/`01` 常识与转弯。

- **四把刀逐刀来**：① 张力/矛盾 → ② 假设反转 → ③ JTBD → ④ 文化位移。Round A 发散（每刀 3–5 候选，问「哪条让你咯噔一下？哪条是废话？」）→ Round B 收敛 → Round C 反驳（三个具名反方，可派子 agent）→ Round D 定稿，过六点标尺，不过退回最弱维重逼，**绝不降标尺**。
...
- 引导段（阶段 1、5）AI 先问后给，不替用户下关键结论。
- 每条断言带 `（来源·级别）`；测算每个数字带级别或假设。
- 一把刀只钻一个裂缝；锚点原则上一个项目一个，runner-up 留在候选墙。
