...
- `references/insight-engine.md` — 四个洞察切入点 + A→D 共创 + 反驳（阶段 4 必读）
...
- **做**：① 扫描竞品 + 平台核心 Message（建坐标系，不是抄）。② 用品类定位框架定位本品角色 + 找**可占领的空位**。③ **洞察先行检查点**：当场拦住「照搬优秀做法」的本能，问「它衰减到哪了？反面/盲区在哪？我有没有资格占空位？」。**洞察先行不写下来不准进阶段 2。**
- **产出**：`00-positioning.md`（用 `templates/positioning-map.md`）。

## 阶段 2 · Brief 解码 + 数据营销 Brief ⚙️ 执行

- **做**：① 用 `templates/brief-decode.md` 跑出信息完整性审查 / 补充问题清单(🔴🟡🟢) / 客户数据索取清单 + 时间机器加项『品类常识清单』『初步张力假设』（输入素材接阶段 1 的洞察先行结论）。② 产出 `templates/data-marketing-brief.md`：一份给**数据营销同事**的取数单（云图/灵犀/星图按空位定向取数，不要全量）。
...
> 完整玩法读 `references/insight-engine.md`。输入素材＝`03 §10` 异常清单 + `00`/`01` 常识与洞察先行。
